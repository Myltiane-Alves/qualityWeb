...
export const ActionListaProdutosDestino = ({ dadosProdutoDestino }) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: 'Lista de Destino',
  });
   ...
   const exportToPDF = () => {
      ...
      doc.save('lista_promocoes_produtos_destino.pdf');
    };
    ...
    const exportToExcel = () => {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Produtos de Destino');
      XLSX.writeFile(workbook, 'llista_promocoes_produtos_destino.xlsx');
    };
  ...
  const colunasProdutoDestino = [
    ...
    {
      header: "Cod. Barras",
      body: row => <th>{row.NUCODBARRASDESTINO}</th>,
      sortable: true,
    },
    {
      header: "Produtos",
      body: row => <th>{row.DSPRODUTODESTINO}</th>,
      sortable: true,
    },
    {
      header: 'Opções',
