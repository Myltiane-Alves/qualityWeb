...
import { ActionEditarEstruturaModal } from "./actionEditarEstruturaModal";
...
export const ActionListaSubGrupoEstrutura = ({ dadosSubGrupo }) => {
  ...
  const dados = dadosSubGrupo.map((item, index) => {
    ...
    return {
      ...
      CODGRUPOESTRUTURA: item.CODGRUPOESTRUTURA,
  ...
  const colunasGrupos = [
    ...
    },
    {
      ...
      body: row => {
        return (
          <p>{`${row.CODGRUPOESTRUTURA} - ${row.DSGRUPOESTRUTURA}`}</p>
        )
