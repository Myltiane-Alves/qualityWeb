...
import { ActionListaVendaCLiente } from "../ActionCliente/actionListaVendaCliente";
import { ActionCadastroClienteVoucherCPF } from "../ActionClienteCPF/actionCadastroClienteVoucheCPF";
import { ActionCadastroClienteVoucherCNPJ } from "../ActionClienteCNPJ/actionCadastroClienteVoucheCNPJ";
...
export const ActionPesquisaCreateVoucherCliente = ({
  ...
  const [tabelaVisivelVoucherSelecionados, setTabelaVisivelVoucherSelecionados] = useState(true);
  ...
  const { onAuthFuncionarioCreate, isLoggedIn, usuarioAutorizado } = useAuthFuncionarioCreate({ dadosVoucherLogin, usuarioLogado }) || {};

  // console.log(colunaSelecionada, "colunaSelecionada")
  const handleClickLogin = async () => {
    console.log(colunaSelecionada, "colunaSelecionada")
    if (colunaSelecionada) {
      setDadosVoucherLogin(colunaSelecionada);
      console.log(dadosVoucherLogin, "dados do login")
      onAuthFuncionarioCreate(() => handleEdit(colunaSelecionada.IDVOUCHER), row);
    }
  }
  ...
  const { data: dadosEmpresasVoucher = [], error: errorFornecedor, isLoading: isLoadingFornecedor, refetch } = useQuery(
    'empresasVoucher',
    async () => {
      const response = await get(`/empresasVoucher?idSubGrupoEmpresa=${usuarioLogado?.IDGRUPOEMPRESARIAL}`);
      return response.data;
    },
    { enabled: Boolean(usuarioLogado?.IDGRUPOEMPRESARIAL), staleTime: 5 * 60 * 1000, cacheTime: 5 * 60 * 1000 } // 5 minutos
  )


  const fetchListaVendasClientes = async () => {
    try {
      const urlApi = `/lista-venda-cliente?idSubGrupoEmpresarial=${usuarioLogado?.IDGRUPOEMPRESARIAL}&idEmpresa=${empresaSelecionada}&dataPesquisaInicio=${dataPesquisaInicio}&dataPesquisaFim=${dataPesquisaFim}&cpfOUidVenda=${cpf}&nnf=${numeroNF}&serie=${serie}`;
    ...
    <Fragment>
      <div className="">
        ...
        <ActionMain
          ...
          placeHolderInputFieldCodBarra="Digite o CPF ou CNPJ ou Código de Barras"
          ...
          onButtonClickVendasEstrutura={handleClickLogin}
          corVendasEstrutura="info"
          iconVendasEstrutura={MdAdd}
          styleVendasEstrutura={{ display: btnVisivel == false ? 'none' : 'block' }}
        />
      </div>


      <ActionCadastroClienteVoucherCPF show={modalCadastroClienteCPF} handleClose={() => setModalCadastroClienteCPF(false)}  />

      <ActionCadastroClienteVoucherCNPJ show={modalCadastroClienteCNPJ}  handleClose={() => setModalCadastroClienteCNPJ(false)}  />
