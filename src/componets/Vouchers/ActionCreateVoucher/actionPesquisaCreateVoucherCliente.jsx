...
export const ActionPesquisaCreateVoucherCliente = ({
  ...
  const fetchListaEmpresasVouchers = async () => {
    try {
      const urlApi = `/empresasVoucher?idEmpresa=${usuarioLogado.IDEMPRESA}&idSubGrupoEmpresa=${usuarioLogado.IDGRUPOEMPRESARIAL}`;
  ...
  const { data: dadosEmpresasVoucher = [], refetch: refetchListaEmpresaVouchers } = useQuery(
    ...
    {
      enabled: Boolean(usuarioLogado?.IDEMPRESA && usuarioLogado?.IDGRUPOEMPRESARIAL), 
    }
  );
