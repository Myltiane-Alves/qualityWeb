...
export const ListaDeVendasAtivas = ({empresaUsuario, dataConsulta}) => {
  ...
  const getTabelas = async () => {
    if(empresaUsuario.length > 0) {
      ...
      try {
        const response = await get(`/vendaAtiva?idEmpresa=${idEmpresa}&dataConsulta=${dataConsulta}`)
        if (response.data && response.data.length > 0) {
          setDadosExemplos(response.data)
        }
        // console.log(response.data, "vendaAtiva")
      } catch (error) {
        ...
        <Accordion defaultActiveKey="0" className="col-xl-12" >
          <Accordion.Item eventKey="0" id="panel-1" className="panel" >
            <Accordion.Header className="panel-hdr tituloListVendasCaixa" >
              {/* <h2>
                Lista de Vendas Ativas <span className="fw-300"><i><?= $NOFANTASIA; ?></i></span>
              </h2> */}
              <h2>
            ...
            <Accordion.Body className="panel-container show">
              <div className="panel-content">
                ...
                <div id="resultadoativas">

                  <div id="dt-basic-pesqvendas_wrapper" className="dataTables_wrapper dt-bootstrap4 no-footer">
