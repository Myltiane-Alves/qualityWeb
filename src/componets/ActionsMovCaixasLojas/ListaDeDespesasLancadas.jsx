...
export const ListaDeDespesasLancadas = ({empresaUsuario, dataConsulta}) => {
  ...
  const getTabelas = async () => {
    if(empresaUsuario.length > 0) {
      ...
      try {
        ...
        // const response = await get(`/DetalheDespesas?idEmpresa${idEmpresa}&dataConsulta=${dataConsulta}`)
        if (response.data && response.data.length > 0) {
          setDadosExemplos(response.data)
        }
        console.log(response.data, "DetalheDespesas")
      } catch (error) {
        ...
        <Accordion defaultActiveKey="0" className="col-xl-12" >
          <Accordion.Item eventKey="0" id="panel-1" className="panel" >
            <Accordion.Header className="panel-hdr tituloListVendasCaixa" >
              {/* <h2>
                 Lista de Despesas Lançadas  <span className="fw-300"><i><?= $NOFANTASIA; ?></i></span>
              </h2> */}
